...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        if (state == "TERMINAL_STATE" or len(legal_actions) == 0):
            return 0
        q_values = []
    ...
    def computeActionFromQValues(self, state):
        ...
        maximum_action = max(actions, key= lambda x: self.getQValue(state, x))
        a_max = [action for action in actions if self.getQValue(state, action) == self.getQValue(state, maximum_action)]
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def __init__(self, extractor : FeatureExtractor ='IdentityExtractor', **args):
    ...
    def getQValue(self, state, action):
        ...
        for key in features.keys():
            q += features[key] * self.weights[key]
        return q
    def update(self, state, action, nextState, reward):
        """
